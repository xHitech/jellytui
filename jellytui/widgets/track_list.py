...
def column_widths(labels, rows, available, padding=1):
    ...
    limits = {"": (1, 1), "Nome": (20, 72), "Artista / tipo": (13, 40),
              "Álbum": (5, 40), "Tempo": (6, 6)}
    widths = []
    ...
    for floors in ({"Nome": 20, "Artista / tipo": 13, "Álbum": 10, "Tempo": 6},
                   {"Nome": 8, "Artista / tipo": 1, "Álbum": 1, "Tempo": 5}, {}):
        for label in ("Álbum", "Artista / tipo", "Nome", "Tempo", *labels):
...
class TrackList(DataTable, inherit_bindings=False):
    ...
    COLUMN_LABELS = ("", "Nome", "Artista / tipo", "Álbum", "Tempo")
    ...
    def show_items(self, items, playing_id=None):
        self.items = list(items)
        self.playing_id = playing_id
        self._display_rows = []
        for item in self.items:
            values = ("▶" if item.id == playing_id else "", ("♥ " if item.favorite else "") + item.name,
                         item.artist or {"Category": "Abrir", "MusicArtist": "Artista", "MusicAlbum": "Álbum", "Folder": "Pasta"}.get(item.kind, item.kind), item.album,
                         time_label(item.duration) if item.is_track else "")
            cells = dict(zip(TrackList.COLUMN_LABELS, values))
            self._display_rows.append(tuple(
                Text(cells[label], no_wrap=True, overflow="ellipsis") for label in self.COLUMN_LABELS
            ))
    ...
    def _fit_columns(self, force=False):
        ...
        widths = column_widths(self.COLUMN_LABELS, self._display_rows, available, self.cell_padding)
        ...
        for label, width in zip(self.COLUMN_LABELS, widths):
